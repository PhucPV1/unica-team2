...
const SiteController = {
    ...
    try {
      ...
      if (token) {
        try {
          ...
          if (user) {
            const accessToken = jwt.sign({ _id: user._id }, `${process.env.signature}`, { expiresIn: "10s" })
            const refreshToken = jwt.sign({ _id: user._id }, `${process.env.signature}`, { expiresIn: "10h" })
      ...
      // token not found
        ...
        return res.status(401).render("error", { err: "", message: "Xin vui lòng đăng nhập để truy cập trang này" })
      }
    } catch (err) {
  ...
  logout: async (req, res) => {
    ...
    try {
      ...
      res.render("error", { err, message: "Có lỗi khi nhận dữ liệu từ server, xin thử lại" })
    }
  },
}
