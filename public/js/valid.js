function Validator(options) {
      ...
      if (isFormValid) {
        ...
        if (typeof options.onSubmit === 'function') {
          ...
          var validInputs = formElement.querySelectorAll(
            '[name]:not([disabled]):not(#g-recaptcha-response)',
          );
          var outputValues = Array.from(validInputs).reduce(function (
            values,
            input,
          ) {
            switch (input.type) {
  ...
  var errorFormGroupCaptcha = getParent(
    captchaElement,
    options.formGroupSelector,
  );
  var formMessageElement = errorFormGroupCaptcha.querySelector(
  ...
  return {
    ...
    check: function (value) {
      return value.length >= min
        ? undefined
        : message || `Please enter a password longer than ${min} characters `;
    },
  };
};
...
Validator.isConfirmPassword = function (selector, checkConfirm, message) {
  return {
    selector: selector,
    check: function (value) {
      return value === checkConfirm()
        ? undefined
        : message || `Please re-enter the matching password`;
    },
  };
};
...
function validateCaptcha() {
  ...
  var errorFormGroupCaptcha =
    document.querySelector('.g-recaptcha').parentElement;
