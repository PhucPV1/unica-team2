...
async function getComment(){
  ...
  try {
    ...
    startFromCmt+=limit;
    if (startFromCmt>=data.length)
      loadmoreBtnCmt.style.display='none';
  } catch (error) {
    console.log(error);
  }
}
...
async function getCourse(){
  ...
  try {
    ...
    if (data.end)
      loadmoreBtnSimCourse.style.display='none';
  } catch (error) {
    console.log(error);
  } 
}
